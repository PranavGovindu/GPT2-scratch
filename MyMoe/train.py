...
class ModelTrainer:
    ...
    def _generate_sample_text(self):
        ...
        for i in range(num_samples):
            ...
            if self.writer:
                ...
                self.writer.add_text(f'GeneratedText/Epoch{self.state.epoch+1}Sample{i+1}', tb_text, self.state.global_step)
    ...
    def save_checkpoint(self, is_best: bool = False):
        ...
        filename_stem = "best_model" if is_best else f"checkpoint_epoch_{self.state.epoch+1}step{self.state.global_step}"
    ...
    def train(self):
        ...
        tensorboard_port = getattr(self.args, 'tensorboard_port', 6006)
        self.tb_process = launch_tensorboard(str(self.tb_dir), port=tensorboard_port)
        # ... (rest of the initial setup as before) ...
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize TensorBoard SummaryWriter: {e}. TB logging disabled.")
            self.writer = None
        ...
        for epoch in range(self.state.epoch, self.args.epochs):
            ...
            # Iterate directly over the train_loader, NO tqdm EPOCH BAR
            for batch_idx, batch in enumerate(self.train_loader): # Added enumerate for batch_idx
                try:
                    loss_dict, grad_norm, tps = self.training_step(batch)
                except Exception as e:
                    self.logger.error(f"Error in training_step G:{self.state.global_step} E:{epoch+1} B:{batch_idx}: {e}", exc_info=True)
                    continue 

                current_lr = self.optimizer.param_groups[0]['lr']

                if self.state.global_step % self.args.log_every_n_steps == 0:
                    # Pass batch_idx for context if needed by log_console_step, though current format doesn't use it
                ...
                if (self.val_loader and self.args.eval_every_n_steps > 0 and
                    ...
                    if val_loss is not None: 
                        ...
                        if self.args.early_stopping_patience > 0 and self.state.epochs_no_improve >= self.args.early_stopping_patience:
                            self.logger.info(f"Early stopping: {self.state.epochs_no_improve} evals no improvement.")
                            self.finalize_training(self.training_start_time)
                            return 
                
                # --- Sample Generation Logic (as before) ---
            ...
            # --- End of Epoch Summary (as before) ---
            ...
            if self.writer:
                self.writer.add_scalar('Loss/train_epoch_avg', avg_epoch_train_loss, self.state.epoch + 1)
    ...
    # ... (rest of the ModelTrainer class: evaluate, _generate_sample_text, save_checkpoint, _calculate_grad_norm, training_step, estimate_training_requirements, finalize_training) ...
    # The setup_logging method from the previous response should be used.
    ...
    def log_console_step(self, loss_dict: Dict[str, float], lr: float, grad_norm: float, tps: float, batch_idx: int):
        """Log compact training step info to console: Step Epoch Loss AuxLoss AvgEpochLoss GradNorm LR TPS"""
        # Main loss for the "Loss" field.
        current_loss_to_display = loss_dict['main']

        # Average epoch loss so far
        avg_epoch_loss = (self.state.train_loss_sum / self.state.train_tokens) if self.state.train_tokens > 0 else float('nan')

        # Steps per epoch and step in epoch
        steps_per_epoch = len(self.train_loader) if hasattr(self, 'train_loader') and self.train_loader else 0
        step_in_epoch = (batch_idx + 1) if steps_per_epoch > 0 else 0

        log_msg = (
            f"Step: {step_in_epoch}/{steps_per_epoch} | "
            f"Epoch: {self.state.epoch+1:<3} | "
            f"Loss: {current_loss_to_display:<7.4f} | "
        )

        # Add AuxLoss if present
        if 'aux' in loss_dict and loss_dict['aux'] is not None:
            log_msg += f"AuxLoss: {loss_dict['aux']:<7.4f} | "

        # Add average epoch loss so far
        log_msg += f"AvgEpochLoss: {avg_epoch_loss:<7.4f} | "
